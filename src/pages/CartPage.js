            ...
            <div className="mt-10 flex flex-col xl:flex-row jusitfy-center items-stretch  w-full xl:space-x-8 space-y-4 md:space-y-6 xl:space-y-0">
                ...
                <div className="row card flex-row" key={p._id}>
                  <div className="mt-4 md:mt-6 flex  flex-col md:flex-row justify-start items-start md:items-center md:space-x-6 xl:space-x-8 w-full ">
                        ...
                        <div className="border-b border-gray-200 md:flex-row flex-col flex justify-between items-start w-full  pb-8 space-y-4 md:space-y-0">
                            <div className="w-full flex flex-col justify-start items-start space-y-8">
                                <h3 className="text-xl xl:text-2xl font-semibold leading-6 text-gray-800 flex md:w-[16rem]">{p.name}</h3>
                                <div className="flex justify-start items-start flex-col space-y-2">
                                    <p className="text-sm leading-none text-gray-800">
                                    <p>{p.description.substring(0, 30)}</p>
                                    </p>
                                    <button
                      className="bg-red-600 text-white font-bold rounded-sm py-2 px-4"
                      ...
                      Remove
                            ...
                            <div className="flex justify-between space-x-8 items-start w-full">
                                <p className="text-base xl:text-lg leading-6">
                                    ${p.price} <span className="text-red-300 line-through"> ${p.price+10}</span>
                                </p>
                                <p className="text-base xl:text-lg leading-6 text-gray-800">01</p>
                                <p className="text-base xl:text-lg font-semibold leading-6 text-gray-800">${p.price}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex justify-center md:flex-row flex-col items-stretch w-full space-y-4 md:space-y-0 md:space-x-6 xl:space-x-8">
                        <div className="flex flex-col px-4 py-6 md:p-6 xl:p-8 w-full bg-gray-50 space-y-6   ">
                            ...
                            <div className="flex justify-between items-center w-full">
                                ...
                                <p className="text-base font-semibold leading-4 text-gray-600">{totalPrice()}.00</p>
                            </div>
                        </div>
                        ...
                        <div className="flex justify-start items-center space-x-4 flex gap-4">
                                    <div class="w-8 h-8">
                                        <img class="w-full h-full" alt="logo" src="https://i.ibb.co/L8KSdNQ/image-3.png" />
                                           </div><h3 className="text-xl font-semibold leading-5 text-gray-800">Payment</h3>
                  ...
                  <>
                    ...
                    >
                      {loading ? "Processing ...." : <><div className="bg-green-600 text-white font-bold px-3 py-3 rounded-ld">Make Payment</div></>}
                    </button>
                  </>
                )}
                        </div>
                    ...
                    <div className="flex  flex-col md:flex-row xl:flex-col justify-start items-stretch h-full w-full md:space-x-6 lg:space-x-8 xl:space-x-0 ">
                        <div className="flex flex-col justify-start items-start flex-shrink-0">
                            <div className="flex justify-center  w-full  md:justify-start items-center space-x-4 py-8 border-b border-gray-200">
                                ...
                                <div className=" flex justify-start items-start flex-col space-y-2">
                                    <p className="text-base font-semibold leading-4 text-left text-gray-800"> {!auth?.user
                ? "Hello Guest"
                : `Hello  ${auth?.token && auth?.user?.name}`}</p>
                            ...
                            <div className="flex justify-center  md:justify-start items-center space-x-4 py-4 border-b border-gray-200 w-full">
                                ...
                                <p className="cursor-pointer text-sm leading-5 text-gray-800"> {!auth?.user
                ? "login to purchase products"
                : ` ${auth?.token && auth?.user?.email}`}</p>
                            </div>
                        </div>
